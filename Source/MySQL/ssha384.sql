/*
 * The original string is hard to guess SHA384.
 *
...
DROP FUNCTION IF EXISTS ssha384//
CREATE FUNCTION ssha384 (plain TEXT, salt TEXT)
...
BEGIN
    ...
    DECLARE result TEXT;
    DECLARE idx INT;
    DECLARE hash_length INT DEFAULT 384;

    -- 平文のハッシュを取得
    IF plain IS NULL THEN
        SET plain = '';
    END IF;
    SET encoded = SHA2(plain, hash_length);
    IF salt IS NULL THEN
        SET salt = '';
    END IF;
    SET encoded_salt = SHA2(salt, hash_length);
    ...
    WHILE repeat_count > idx DO
        ...
        -- ハッシュを取得する
        SET result = SHA2(encoded, hash_length);

        -- ハッシュ値をセットする
        SET encoded = result;
        SET encoded_salt = SHA2(encoded_salt, hash_length);
